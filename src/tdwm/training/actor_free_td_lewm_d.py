"""Standalone training integration for method D (teacher-score weighting)."""
...
def _compute_loss(context: FrozenTDContext) -> FrozenTDLoss:
    ...
    weighted = teacher_goal_weighted_td_loss(
    ...
    return FrozenTDLoss(
        loss=weighted.loss,
        metrics={
            f"{stage}/teacher_goal_score_mean": weighted.teacher_score.mean(),
            f"{stage}/teacher_goal_score_std": weighted.teacher_score.std(
                unbiased=False
            ),
...
def load_actor_free_td_lewm_d_training_protocol(
    path: str | Path,
) -> dict[str, Any]:
    return load_frozen_actor_free_td_training_protocol(path, spec=SPEC)


def validate_actor_free_td_lewm_d_training_protocol(
    protocol: dict[str, Any],
) -> None:
    validate_frozen_actor_free_td_training_protocol(protocol, spec=SPEC)


def train_actor_free_td_lewm_d(**kwargs: Any) -> dict[str, Any]:
...
__all__ = [
    ...
    "validate_actor_free_td_lewm_d_training_protocol",
]
