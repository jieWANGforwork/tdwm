"""Transition-level frozen-cache data access for Actor-Free TD-LeWM V1.
...
from tdwm.methods.actor_free_td_lewm_v1 import V1_RAW_ACTION_DIM
...
def _canonical_item_indices(
    ...
    *,
    ...
    if np.any(canonical < 0) or np.any(canonical >= int(length)):
        raise IndexError("A V1 transition index is out of range.")
    return canonical


@dataclass(frozen=True)
class ReachableFutureLatentsV1:
...
class FrozenActorFreeTDV1TransitionDataset:
    """Expand selected frozen clips into individual V1 TD transitions.

    Args:
        clip_dataset: Validated row-backed clip dataset.
        clip_indices: Indices in ``clip_dataset`` belonging to one split.
        first_current_index: First TD position in each clip.  V1 fixes this to
    ...
    def __init__(
        ...
        *,
        first_current_index: int = V1_FIRST_CURRENT_INDEX,
        ...
        if store.embed_dim != V1_STATE_DIM:
            raise ValueError("V1 requires 192-dimensional frozen LeWM states.")
        if store.action_block_dim != V1_RAW_ACTION_DIM:
            raise ValueError("V1 requires 25-dimensional normalized action blocks.")
        first_current = int(first_current_index)
        if first_current != V1_FIRST_CURRENT_INDEX:
            raise ValueError("V1 fixes first_current_index to 3.")
        if store.history_frames != first_current:
            raise ValueError("Frozen-store history_frames differs from V1.")
        ...
        if clip_dataset.num_steps < first_current + 2:
            raise ValueError("Each V1 clip must leave a current and next state.")
        ...
        if np.any(dense_final_rows >= store.total_rows):
            raise IndexError("A selected V1 clip exceeds the frozen store.")
        ...
        if not np.array_equal(clip_episodes, actual_start_episodes) or not (
            ...
            raise ValueError("A selected V1 clip crosses an episode boundary.")
        ...
        for chunk_start in range(0, selected.size, chunk_size):
            ...
            if np.any(np.asarray(store.episode_ids[rows]) != episodes) or np.any(
                ...
                raise ValueError("A V1 TD pair crosses an episode boundary.")
        ...
        if expanded_rows.size == 0:
            raise ValueError("The selected clips contain no legal V1 transitions.")
        ...
        if np.any(future_end_rows < global_rows + frame_skip):
            raise RuntimeError("A V1 future-goal range is empty.")
        if np.any((future_end_rows - global_rows) % frame_skip != 0):
            raise RuntimeError("A V1 future-goal range is not frame aligned.")
    ...
    def __getitems__(
        ...
        if not np.isfinite(actions_array).all():
            raise RuntimeError("A pre-indexed V1 current action became non-finite.")
        ...
        if not np.array_equal(observed_terminals, terminals):
            raise RuntimeError("A pre-indexed V1 terminal marker changed.")
        np.nan_to_num(
            next_actions_array,
            copy=False,
            nan=0.0,
...
def sample_reachable_future_latents_v1(
    ...
    *,
    ...
    if generator is not None and torch.device(generator.device).type != "cpu":
        raise ValueError("The V1 future-goal generator must be a CPU generator.")
    ...
    if np.any(store.episode_ids[ends] != store.episode_ids[rows]):
        raise ValueError("A V1 future-goal bound crosses an episode boundary.")
    ...
    if np.any(store.episode_ids[sampled_rows] != store.episode_ids[rows]):
        raise ValueError("A sampled V1 future goal crosses an episode boundary.")

    latents = torch.from_numpy(
        np.array(store.latents[sampled_rows], dtype=np.float32, copy=True)
    )
    target_device = torch.device(device)
    return ReachableFutureLatentsV1(
