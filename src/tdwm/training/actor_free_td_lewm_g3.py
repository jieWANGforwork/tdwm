"""Standalone training integration for G3 prefix-marginal weighting."""
...
def _compute_loss(context: FrozenTDContext) -> FrozenTDLoss:
    ...
    weighted = action_prefix_marginal_advantage_td_loss(
        context.successor,
    ...
    metrics = {
        ...
        f"{stage}/prefix_marginal_mean": weighted.marginal_scores.mean(),
        f"{stage}/prefix_marginal_std": weighted.marginal_scores.std(unbiased=False),
        f"{stage}/prefix_marginal_advantage_mean": weighted.advantage.mean(),
        f"{stage}/prefix_marginal_advantage_std": weighted.advantage.std(
            unbiased=False
        ),
        **frozen_weight_metrics(stage, weighted.weight_diagnostics),
    }
    for marginal_index in range(weighted.marginal_scores.shape[-1]):
        metrics[
            f"{stage}/prefix_delta_{marginal_index + 2}_mean"
        ] = weighted.marginal_scores[:, marginal_index].mean()
    return FrozenTDLoss(loss=weighted.loss, metrics=metrics)
...
def load_actor_free_td_lewm_g3_training_protocol(
    path: str | Path,
) -> dict[str, Any]:
    return load_frozen_actor_free_td_training_protocol(path, spec=SPEC)


def validate_actor_free_td_lewm_g3_training_protocol(
    protocol: dict[str, Any],
) -> None:
    validate_frozen_actor_free_td_training_protocol(protocol, spec=SPEC)


def train_actor_free_td_lewm_g3(**kwargs: Any) -> dict[str, Any]:
...
__all__ = [
    ...
    "validate_actor_free_td_lewm_g3_training_protocol",
]
