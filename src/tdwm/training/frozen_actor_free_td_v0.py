"""Standalone frozen-LeWM training runtime for Actor-Free TD-JEPA V0.

V0 is deliberately separate from the V-1 frozen successor runtime.  The
...
class ActorFreeTDLeWMV0Spec:
    """Identity of one independently runnable V0 method."""
    ...
    def __post_init__(self) -> None:
        if self.method != f"{METHOD_FAMILY}_{self.variant}":
            raise ValueError("V0 method names must end in their exact variant.")
        if self.variant not in SUPPORTED_VARIANTS:
            raise ValueError(f"Unsupported V0 variant: {self.variant!r}.")
        if self.requires_neighbor_index != (self.variant == "g1"):
            raise ValueError("Only V0 G1 uses the training neighbor index.")


V0_SPECS = {
    variant: ActorFreeTDLeWMV0Spec(
...
def load_actor_free_td_lewm_v0_training_protocol(
    path: str | Path,
    *,
    ...
    if not isinstance(protocol, dict):
        raise ValueError("V0 training protocol must contain a mapping.")
...
def validate_actor_free_td_lewm_v0_training_protocol(
    protocol: dict[str, Any],
    *,
    spec: ActorFreeTDLeWMV0Spec,
) -> None:
    """Validate the V0 architecture and experiment boundary without V-1 locks."""
    ...
    if protocol.get("seeds") != [0, 42, 3072]:
        raise ValueError("V0 uses the comparison seeds [0, 42, 3072].")
    if protocol.get("runtime", {}).get("stable_worldmodel_version") != "0.1.1":
        raise ValueError("V0 requires stable-worldmodel 0.1.1.")
    ...
    if (
        ...
        raise ValueError("V0 Cube alignment requires frame_skip=5 and data offset 3.")
    ...
    if int(protocol.get("model", {}).get("embed_dim", 0)) != V0_STATE_DIM:
        raise ValueError("V0 uses the baseline LeWM 192D latent.")
    ...
    if epochs * steps != FORMAL_OPTIMIZER_UPDATES:
        raise ValueError("V0 formal runs require exactly 127960 optimizer updates.")
    if training.get("model_compile") is not False:
        raise ValueError("V0 comparison keeps model compilation disabled.")
    optimizer = protocol.get("optimizer", {})
    if float(optimizer.get("world_model_learning_rate", -1.0)) != 0.0:
        raise ValueError("The V0 LeWM world model is frozen.")
...
def _build_v0_training_module(
    ...
    *,
    ...
    class ActorFreeTDLeWMV0TrainingModule(pl.LightningModule):
        def __init__(self) -> None:
            super().__init__()
            self.model = world_model.requires_grad_(False)
            self.model.eval()
            cfg = protocol["predictor"]
            self.predictor = ActorFreeTDJEPAPredictorV0(
            ...
            if spec.requires_neighbor_index != (neighbor_index is not None):
                raise ValueError("Only V0 G1 accepts a neighbor index.")
        ...
        def _score_neighbors(
            ...
            if self.neighbor_index is None:
                raise RuntimeError("V0 G1 is missing its neighbor index.")
            ...
            with torch.no_grad():
                positive = tdjepa_goal_score_v0(
                    self.predictor(state, action, task), task
                )
                ...
                neighbor_scores = tdjepa_goal_score_v0(
                    self.predictor(neighbor_state, neighbors.actions, neighbor_task),
                    neighbor_task,
                )
            if positive.shape != (count,):
                raise RuntimeError("V0 G1 positive score alignment failed.")
        ...
        def _score_prefixes(
            ...
            prefixes = build_zero_mean_action_prefixes(action.detach())
            prefix_state = state.unsqueeze(1).expand(-1, 5, -1)
            prefix_task = task.unsqueeze(1).expand(-1, 5, -1)
            with torch.no_grad():
                return tdjepa_goal_score_v0(
                    self.predictor(prefix_state, prefixes, prefix_task), prefix_task
                )
        ...
        def _method_loss(
            ...
            *,
            ...
            if self.variant == "g1":
                ...
                return output.loss, metrics
            prefix_scores = self._score_prefixes(state, action, task)
            if self.variant == "g2":
        ...
        def _forward_loss(self, batch: dict[str, Any], stage: str) -> torch.Tensor:
            names_and_dims = {
                "state": V0_STATE_DIM,
                "action": V0_ACTION_DIM,
                "next_state": V0_STATE_DIM,
                "next_action": V0_ACTION_DIM,
            }
            ...
            for name, dimension in names_and_dims.items():
                ...
                if not isinstance(value, torch.Tensor) or value.ndim != 2:
                    raise RuntimeError(
                        f"V0 transition field {name!r} must have shape [B,{dimension}]."
                    )
                if value.shape[-1] != dimension or not bool(
                    torch.isfinite(value).all()
                ):
                    raise RuntimeError(
                        f"V0 transition field {name!r} is malformed or non-finite."
                    )
                if batch_size is None:
                    batch_size = int(value.shape[0])
                elif value.shape[0] != batch_size:
                    raise RuntimeError("V0 transition fields have different batch sizes.")
                tensors[name] = value
            ...
            if (
                ...
                raise RuntimeError("V0 terminal must be a boolean [B] tensor.")
            for name, value in {
                ...
                if (
                    ...
                    raise RuntimeError(f"V0 {name} must be an integer [B] tensor.")
            if self.latent_store is None:
                matched_goals = batch.get("_tdwm_matched_goal")
                if (
                    not isinstance(matched_goals, torch.Tensor)
                    or matched_goals.shape != (batch_size, V0_TASK_DIM)
                ):
                    raise RuntimeError(
                        "V0 requires its frozen latent store to sample real goals."
                    )
                ...
                matched_goals = sample_reachable_future_latents_v0(
                    self.latent_store,
                    global_rows,
                    future_end_rows,
            ...
            td_batch = build_tdjepa_td_batch_v0(
                self.predictor,
    ...
    return ActorFreeTDLeWMV0TrainingModule()
...
def _build_export_callback(
    ...
    *,
    ...
    class V0ExportCallback(pl.Callback):
...
def train_actor_free_td_lewm_v0(
    *,
    spec: ActorFreeTDLeWMV0Spec,
    ...
    """Train one V0 method from the same audited frozen LeWM artifacts."""

    protocol = load_actor_free_td_lewm_v0_training_protocol(
    ...
    if initial_world_model_checkpoint_path is None:
        raise ValueError("V0 requires --initial-world-model-checkpoint.")
    if frozen_latent_store_path is None or split_indices_path is None:
        raise ValueError("V0 requires frozen latent store and split indices.")
    if spec.requires_neighbor_index != (neighbor_index_path is not None):
        raise ValueError("Only V0 G1 requires --neighbor-index.")
    ...
    if dataset_source["format"] != "lance":
        raise ValueError("V0 frozen training requires the audited Lance dataset.")
    ...
    if len(source_dataset.lengths) != int(dataset_cfg["expected_episodes"]):
        raise ValueError("Dataset episode count differs from V0 protocol.")
    if int(np.asarray(source_dataset.lengths).sum()) != int(
        dataset_cfg["expected_transitions"]
    ):
        raise ValueError("Dataset transition count differs from V0 protocol.")
    ...
    train_set = FrozenActorFreeTDV0TransitionDataset(
        clip_dataset,
        train_indices,
        first_current_index=int(protocol["sequence"]["history_frames"]),
    )
    validation_set = FrozenActorFreeTDV0TransitionDataset(
        clip_dataset,
        validation_indices,
        first_current_index=int(protocol["sequence"]["history_frames"]),
    )
    ...
    if neighbor_index_path is not None:
        neighbor_index = StateNeighborActionIndex(
            ...
            expected_action_block_dim=V0_ACTION_DIM,
        ...
        if neighbor_index.manifest.get("training_split_sha256") != split_manifest[
            ...
            raise ValueError("V0 G1 neighbor index uses another training split.")
        neighbor_info = {
    ...
    if source_hash != protocol["pretrained_world_model"]["checkpoint_sha256"]:
        raise ValueError("V0 pretrained checkpoint SHA differs from protocol.")
    if source_hash != store_info["pretrained_checkpoint_sha256"]:
        raise ValueError("V0 latent store was encoded by another LeWM checkpoint.")
    ...
    if formal_epoch_steps != len(train_loader):
        raise RuntimeError(
            "V0 transition sampler must yield exactly optimizer_steps_per_epoch."
        )
    ...
    if resume != "never" and last_checkpoint.is_file():
        if not manifest_path.is_file():
            raise RuntimeError("Cannot verify V0 resume without its manifest.")
        previous = json.loads(manifest_path.read_text())
        if (
            previous.get("protocol_sha256") != protocol_hash
            or previous.get("seed") != seed
            or previous.get("frozen_latent_store", {}).get("manifest_sha256")
            != store_info["manifest_sha256"]
        ):
            raise RuntimeError("Refusing to resume an incompatible V0 run.")
