"""Controlled Cube evaluation for Actor-Free TD-LeWM V2 C."""
...
def validate_actor_free_td_lewm_v2_c_evaluation_protocol(protocol) -> None:
    validate_actor_free_td_v2_evaluation_protocol(protocol, spec=METHOD_SPEC)


def load_actor_free_td_lewm_v2_c_evaluation_protocol(
    path: str | Path,
) -> dict[str, Any]:
    return load_actor_free_td_v2_evaluation_protocol(path, spec=METHOD_SPEC)


def validate_actor_free_td_lewm_v2_c_checkpoint_protocol(**kwargs) -> None:
    validate_actor_free_td_v2_checkpoint_protocol(spec=METHOD_SPEC, **kwargs)


def configure_actor_free_td_lewm_v2_c_evaluation_mode(protocol, **kwargs):
    return configure_actor_free_td_v2_evaluation_mode(protocol, **kwargs)


def evaluate_actor_free_td_lewm_v2_c(**kwargs) -> dict[str, Any]:
    return evaluate_actor_free_td_v2(
        spec=METHOD_SPEC,
        checkpoint_loader=load_actor_free_td_lewm_v2_c_checkpoint,
        policy_factory=make_actor_free_td_lewm_v2_c_policy,
        **kwargs,
    )
...
__all__ = [
    ...
    "OBJECTIVE_VERSION",
    "VARIANT",
    "configure_actor_free_td_lewm_v2_c_evaluation_mode",
    "evaluate_actor_free_td_lewm_v2_c",
    "load_actor_free_td_lewm_v2_c_evaluation_protocol",
    "validate_actor_free_td_lewm_v2_c_checkpoint_protocol",
    "validate_actor_free_td_lewm_v2_c_evaluation_protocol",
]
