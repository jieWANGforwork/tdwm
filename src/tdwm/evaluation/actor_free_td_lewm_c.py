"""Controlled Cube evaluation for Actor-Free TD-LeWM C."""
...
def validate_actor_free_td_lewm_c_evaluation_protocol(protocol) -> None:
    validate_frozen_actor_free_td_evaluation_protocol(protocol, spec=METHOD_SPEC)


def load_actor_free_td_lewm_c_evaluation_protocol(
    path: str | Path,
) -> dict[str, Any]:
    return load_frozen_actor_free_td_evaluation_protocol(path, spec=METHOD_SPEC)


def validate_actor_free_td_lewm_c_checkpoint_protocol(**kwargs) -> None:
    validate_frozen_actor_free_td_checkpoint_protocol(spec=METHOD_SPEC, **kwargs)


def configure_actor_free_td_lewm_c_evaluation_mode(protocol, **kwargs):
    return configure_frozen_actor_free_td_evaluation_mode(protocol, **kwargs)


def evaluate_actor_free_td_lewm_c(**kwargs) -> dict[str, Any]:
    return evaluate_frozen_actor_free_td(
        spec=METHOD_SPEC,
        checkpoint_loader=load_actor_free_td_lewm_c_checkpoint,
        policy_factory=make_actor_free_td_lewm_c_policy,
        **kwargs,
    )
...
__all__ = [
    ...
    "OBJECTIVE_VERSION",
    "VARIANT",
    "configure_actor_free_td_lewm_c_evaluation_mode",
    "evaluate_actor_free_td_lewm_c",
    "load_actor_free_td_lewm_c_evaluation_protocol",
    "validate_actor_free_td_lewm_c_checkpoint_protocol",
    "validate_actor_free_td_lewm_c_evaluation_protocol",
]
