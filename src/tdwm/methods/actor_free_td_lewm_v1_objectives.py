"""Pure C--G3 objectives for the goal-conditioned actor-free TD-LeWM V1.

V1 keeps objective reduction separate from the predictor and data pipeline.  A
...
VARIANT_FAMILY = "actor_free_td_lewm_v1"
OBJECTIVE_VERSION = 0
ACTION_PREFIX_SLOTS = 5


@dataclass(frozen=True)
class GoalProjectedV1Output:
...
@dataclass(frozen=True)
class GoalValueWeightedV1Output:
...
@dataclass(frozen=True)
class SameFutureGoalAdvantageV1Output:
...
@dataclass(frozen=True)
class NeighborActionAdvantageV1Output:
...
@dataclass(frozen=True)
class PrefixMeanAdvantageV1Output:
...
@dataclass(frozen=True)
class PrefixMarginalAdvantageV1Output:
...
def goal_value_weighted_v1_loss(
    td_targets: torch.Tensor,
    tasks: torch.Tensor,
    goal_mask: torch.Tensor,
    per_transition_td_loss: torch.Tensor,
    *,
    ...
    return GoalValueWeightedV1Output(
...
def same_future_goal_advantage_v1_loss(
    td_targets: torch.Tensor,
    tasks: torch.Tensor,
    goal_mask: torch.Tensor,
    per_transition_td_loss: torch.Tensor,
    *,
    temperature: float,
) -> SameFutureGoalAdvantageV1Output:
    ...
    return SameFutureGoalAdvantageV1Output(
        loss=_weighted_td_loss(per_transition_td_loss, weights),
        per_transition_td_loss=per_transition_td_loss,
...
def neighbor_action_advantage_v1_loss(
    ...
    *,
...
) -> NeighborActionAdvantageV1Output:
    ...
    return NeighborActionAdvantageV1Output(
        loss=_weighted_td_loss(per_transition_td_loss, weights),
        per_transition_td_loss=per_transition_td_loss,
...
def prefix_mean_advantage_v1_loss(
    prefix_scores: torch.Tensor,
    goal_mask: torch.Tensor,
    per_transition_td_loss: torch.Tensor,
    *,
    temperature: float,
) -> PrefixMeanAdvantageV1Output:
    ...
    return PrefixMeanAdvantageV1Output(
        loss=_weighted_td_loss(per_transition_td_loss, weights),
        per_transition_td_loss=per_transition_td_loss,
...
def prefix_marginal_advantage_v1_loss(
    prefix_scores: torch.Tensor,
    goal_mask: torch.Tensor,
    per_transition_td_loss: torch.Tensor,
    *,
    temperature: float,
) -> PrefixMarginalAdvantageV1Output:
    ...
    return PrefixMarginalAdvantageV1Output(
        loss=_weighted_td_loss(per_transition_td_loss, weights),
        per_transition_td_loss=per_transition_td_loss,
        prefix_scores=selected_prefixes,
