...
class ActorFreeTDLeWMV1(nn.Module):
    ...
    def get_cost(
        ...
        if self.score_mode == ROLLOUT_MEAN_SCORE_MODE:
            if horizon != 5:
                raise ValueError(
                    "V1 g_only_f_rollout_mean requires CEM planning horizon=5."
                )
    ...
    def _g_only_f_rollout_mean_cost(
        ...
        """Average G over aligned ``(z_(k-1), E_A(raw_A_k))`` rollout pairs."""
        ...
        expected_shape = (batch, samples, horizon, V1_STATE_DIM)
        if predecessors.shape != expected_shape:
            raise ValueError(
                "V1 rollout-mean predecessor states must align one-to-one with "
        ...
        if scores.shape != (batch, samples, horizon):
            raise ValueError(
                "V1 rollout-mean G scores must have shape (batch, samples, horizon)."
            )
