...
## 执行周期修正（2026-09-13，当前有效）

用户已要求按 LeWM Cube / RP1 的 H5/RH5 执行周期修正 Efficient 与 Efficient Plan：
O25/O50/O100 每次均规划并执行五块，即 25 个原始动作，再从真实新观测重新规划。
总预算仍分别为 50/100/200；目标、评分、网络、训练数据与 checkpoint 不变。
O50 为同一规则的扩展，RP1 Cube 原文报告 O25/O100。

此前从 C 系列继承的 O50/O100 RH1 不是本次要遵循的论文执行周期，
以下旧决策只保留作历史记录，不能继续据此运行新的正式评测。
旧结果保留 RH1 身份；不能修改 manifest 把它们伪装成修正后结果。
新的配对比较须使用同一 RH5 执行设置的 F-only、Eff、EffPlan。

## 历史基线值（2026-09-12；其中 RH1 已被上节取代）
...
| 字段 | 建议值 | 备注 |
|---|---|---|
| `receding_horizons` | `{"25": 5, "50": 5, "100": 5}` | 已于 2026-09-13 确认；每轮执行完整 25 原始步，总预算仍为 2×offset |
