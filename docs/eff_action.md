...
1. 这批清单从全部 10,000 episodes 抽取。**已实测（只读，未训练）**：C–G3 的划分为 `split_indices_seed3072_numsteps19.npz`，是**按行**的 90/10（每 episode 107 行，train 963,000 / val 107,000），且 **10,000 个 episode 全部同时出现在 train 与 validation 两半**，即 C–G3 见过全部 10,000 条 episode。对三档 seed-42 清单逐条比对的结果是：

   | offset | EffAction 见过（训练区 0–7999） | C–G3 见过（训练行覆盖） |
   |---|---|---|
   | O25 | 40/50 | 50/50 |
   | O50 | 40/50 | 50/50 |
   | O100 | 40/50 | 50/50 |

   因此在这批清单上 **EffAction 的 episode 曝光度是 80%，C–G3 是 100%，EffAction 反而低 20 个百分点**。此前「EffAction 绝对成功率偏乐观」的说法方向有误，已更正：偏乐观只相对于「EffAction 自己的 8000–9999 留出评测」成立，**相对于同清单上的 C–G3 并不占便宜**。这个 20 个百分点的曝光度差必须随结果一起报告。
2. 只有「同一批 pair 上 EffAction 与 C–G3 的差值」是受控比较；跨 pair 集合的相减仍然无效。
3. 冻结底座 F（LeWM）自身是否见过这些 episode 需单独记录，本对齐只保证两个方法题目相同。
4. **曝光度不匹配尚未解决**（见上表 80% vs 100%）。要让两个方法真正站在同一条线上，只有一条路：让它们在同一划分上训练。三条候选路径**待用户裁定**：
   - **(i)** G/V/P 改用 C–G3 同款「按行 90/10」划分重训（需改采样器以支持行级 split artifact；且与「按 RP1 训练 0–7999」的原始指令冲突）。
   - **(ii)** 维持现状（当前已落地的做法），报告中同时给出「全 50 对」与「双方都见过的 40 对」两个数字，后者曝光度更接近。
   - **(iii)** 回到 8000–9999 留出评测（`rp1_heldout`），并**在同一批留出对上重跑 C–G3 基线**后再比较。
