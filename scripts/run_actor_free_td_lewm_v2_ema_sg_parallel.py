#!/usr/bin/env python3
"""Fail-closed five-GPU launcher for Actor-Free TD-LeWM V2-EMA-SG.
...
def audited_git_state(repository: Path) -> dict[str, Any]:
    ...
    if dirty:
        raise RuntimeError("V2-EMA-SG launcher requires a clean Git worktree.")
...
"""
    ...
    if runtime.get("stable_worldmodel") != "0.1.1":
        raise RuntimeError("V2-EMA-SG requires stable-worldmodel==0.1.1.")
    return runtime
...
def build_jobs(
    *,
    ...
    if stage == "eval":
        for mode in SCORE_MODES:
            for variant in VARIANTS:
                config = (
                    f"configs/experiment/actor_free_td_lewm_v2_ema_sg_{variant}"
                ...
                argv = (
                    python,
                    f"scripts/evaluate_actor_free_td_lewm_v2_ema_sg_{variant}.py",
                    "--config",
                    config,
                    "--dataset",
                    str(paths.dataset),
    ...
    for variant in VARIANTS:
        config = (
            f"configs/experiment/actor_free_td_lewm_v2_ema_sg_{variant}_cube_train.yaml"
        )
        ...
        argv_list = [
            python,
            f"scripts/train_actor_free_td_lewm_v2_ema_sg_{variant}.py",
            "--config",
            config,
            "--dataset",
            str(paths.dataset),
    ...
    environment.update(
        {
            ...
            "PYOPENGL_PLATFORM": environment.get("PYOPENGL_PLATFORM", render_backend),
        }
    )
...
def _paths(args: argparse.Namespace, repository: Path, short_revision: str) -> Paths:
    ...
    bundle = (
        Path(
            ...
            or artifact_root
            / "outputs"
            / f"actor_free_td_lewm_v2_ema_sg_cg3_{short_revision}"
        )
        .expanduser()
        .resolve()
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    with lock_path.open("a+", encoding="utf-8") as lock:
        try:
            ...
            raise RuntimeError(
                "Another V2-EMA-SG launcher owns this bundle."
            ) from error
