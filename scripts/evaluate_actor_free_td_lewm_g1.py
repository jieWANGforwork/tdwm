...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate independent Actor-Free TD-LeWM G1 with policy-free CEM."
    )
    ...
    parser.add_argument(
        "--score-mode",
        choices=("f_only", "g_only", "f_plus_g"),
        default=None,
    )
...
def main() -> None:
    ...
    if output_dir is None:
        protocol = load_actor_free_td_lewm_g1_evaluation_protocol(args.config)
        output_dir = (
    ...
    result = evaluate_actor_free_td_lewm_g1(
