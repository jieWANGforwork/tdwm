...
def _transition_dataset(
    ...
    transitions = FrozenActorFreeTDV1TransitionDataset(clips, [0, 1])
...
def test_equal_future_bound_tie_chooses_smallest_source_clip_index(
    ...
    transitions = FrozenActorFreeTDV1TransitionDataset(clips, [4, 1])
...
def test_many_overlapping_records_vectorize_to_canonical_unique_rows(
    ...
    def fail_scalar_clip_lookup(*args, **kwargs):
        del args, kwargs
        raise AssertionError("V1 index construction must normalize clips in bulk")

    monkeypatch.setattr(clips, "_clip_start", fail_scalar_clip_lookup)
    transitions = FrozenActorFreeTDV1TransitionDataset(
        clips,
...
def test_reachable_future_sampling_is_matched_and_reproducible(tmp_path: Path) -> None:
    ...
    first = sample_reachable_future_latents_v1(
        store,
        rows,
        ends,
        generator=torch.Generator().manual_seed(91),
    )
    second = sample_reachable_future_latents_v1(
        store,
        rows,
        ends,
        generator=torch.Generator().manual_seed(91),
...
def test_explicit_endpoint_selection_uses_inclusive_end_without_rng(
    ...
    selected = sample_reachable_future_latents_v1(
        store,
        rows,
        ends,
        generator=None,
...
def test_finite_next_action_is_terminal_when_next_next_leaves_episode(
    ...
    store, _ = _build_store(tmp_path / "cache")
    clips = FrozenLatentClipDataset(_SourceDataset(), store)
    transitions = FrozenActorFreeTDV1TransitionDataset(clips, [3])
...
def test_transition_dataset_rejects_non_v1_index_contract(tmp_path: Path) -> None:
    ...
    with pytest.raises(IndexError, match="clip_indices"):
        FrozenActorFreeTDV1TransitionDataset(clips, [len(clips)])
    with pytest.raises(ValueError, match="cannot be empty"):
        FrozenActorFreeTDV1TransitionDataset(clips, [])
